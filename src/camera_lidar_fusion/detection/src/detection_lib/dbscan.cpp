...
void DbScan::process(const Image::ConstPtr & image_detection_grid){
	/*主要采用DBSCAN来进行点云的检测，是将前面数据融合中的image detection grid转换为cv Mat的detection grid，
	送入到DBSCAN中，进行聚类, 获取聚类后的属性信息，如bbox信息，宽高信息，颜色信息等，然后将结果发布出来
	*/
...
void DbScan::runDbScan(cv::Mat grid){
	...
	for(int y = 0; y < grid.rows; y++){
		for(int x = y; y < grid.cols - x; x++){
			...
			//然后将点云标记为核心点、边界点还是噪声点，
			//这里首先语义信息找出kernel内具有相同语义信息的cell
			//
			...
			while(!neighbor_queue.empty()){
				...
				//寻找kernel内具有相同语义信息的cell，然后计算相邻cell的索引，判断相邻的cell是否超出边界
...
void DbScan::filterClusters(const cv::Mat grid){
	...
	//拟合最小的矩形，然后获取矩形框或者边界框的中心点以及长度，宽度
	//通过z方向上的最大最小，判断高目标的高度信息，获取边界框的方向信息
